...
const UserProfilePage = () => {
  ...
  const handleOnsubmit = async (event) => {
    ...
    subscribeOnce(updateUser(user._id, userUpdate), (res) => {
      if (!res) {
        toast("Update fail");
        return;
      }
  ...
  const createPayment = () => {
    ...
    if (typeof amountToPay !== "number" || amountToPay <= 0) return;
    subscribeOnce(createPaymentService({ amount: amountToPay }), (res) => {
      if (!res) return;
      console.log(res);
      if (res.amount) localStorage.setItem("amount", res.amount);
      window.location.href = res.paymentLink;
    });
  };
  return (
    <div className="vw-full mb-5 user-profile-container">
          ...
          <div className="col-span-5 xl:col-span-2">
            <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
              ...
              <div className="font-medium">
                <div>
                  My current moneys:{" "}
                  {user.bag ? parseInt(user.bag, 10).toLocaleString() : 0} (VND){" "}
                </div>
              </div>
